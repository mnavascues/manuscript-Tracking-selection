...
%DIF ADD tracking_selection_manuscript.tex   Thu Nov  3 16:45:13 2022
...
\DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Some methods to perform the joint inference of demography and selection have been proposed by making the assumption that a set of neutral polymorphisms can be distinguished from another set of polymorphisms putatively under selection (usually synonymous }\emph{\DIFadd{vs.}}\DIFadd{\ non-synonymous sites). }\Citet{McDonald:1991} \DIFadd{first proposed to compare polymorphism and divergence between synonymous and non-synonymous sites and soon after \mbox{%DIFAUXCMD
...
Our model is relatively simple, as it only considered the impact of \DIFaddbegin \DIFadd{new }\DIFaddend beneficial mutations, neglecting the effect of background selection and standing variation. Background selection can mimic directional selection because they cause a similar pattern of diversity reduction around the target of selection \citep{Stephan:2010} has been discussed for a long time. Much less has been discussed about the patterns of background selection on temporal data and their differences with selective sweeps. \citet{Cvijovic:2018} showed that neutral alleles linked to less deleterious backgrounds could quickly rise to high frequencies due to purifying selection, which could mimic the temporal signal of a selective sweep. However, \DIFdelbegin \DIFdel{if background selection is not localized to specific regions of the genome, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
