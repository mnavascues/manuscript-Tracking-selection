...
%DIF DEL previous_version.tex                Wed Oct 26 10:46:14 2022
%DIF ADD tracking_selection_manuscript.tex   Tue Nov  8 15:38:31 2022
...
\DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{Some methods to perform the joint inference of demography and selection have been proposed by making the explicit assumption that a set of neutral polymorphisms can be distinguished from another set of polymorphisms putatively under selection (usually synonymous }\emph{\DIFadd{vs.}}\DIFadd{\ non-synonymous sites). }\Citet{McDonald:1991} \DIFadd{first proposed to compare polymorphism and divergence between synonymous and non-synonymous sites and soon after \mbox{%DIFAUXCMD
\citet{Sawyer1992} }\hspace{0pt}%DIFAUXCMD
proposed to model the evolution both types of sites with a Poisson random field. Several developments based on these ideas have been put forward over the years, improving and refining the models \mbox{%DIFAUXCMD
\citep[\emph{e.g.}][]{Boyko:2008, Messer:2013}}\hspace{0pt}%DIFAUXCMD
. Alternatively, other approaches do not make the assumption about the neutral nature of some specific polymorphisms. In these, the effect of selection is assumed to be heterogeneous among loci or genomic regions, and they often have the additional objective to identify the loci or regions under (strong) selection. In the models used by these approaches, it }\DIFaddend is often difficult to calculate the likelihood \DIFdelbegin \DIFdel{of models including demography and selection }\DIFdelend \citep[but see][]{Vitalis:2014ja}. Methods that rely on simulations provide easier alternatives to using likelihood functions \citep{Csillery:2010jd, Schrider:2018ei}. One of the first works that proposed such a strategy addressed the inference of local adaptation \citep{Bazin:2010dv}. With coalescent simulations of an island model, \citet{Bazin:2010dv} estimated demographic parameters and inferred the number of loci under selection. In their simulations, the selection was modeled as locus-specific migration rates in which a selected locus had lower migration rates than neutral loci. However, locus-specific migration rates or effective population size \citep[as in][]{Roux:2016gm, Fraisse:2021gg} represent crude approximations of the selection process. Forward-in-time simulation allows more realistic models of selection. These were used to make inferences on $N_\mathrm{e}$ in the presence of selection by \citet{Sheehan:2016caa} (selective sweeps and balancing selection) and \citet{Johri:2020ee} (background selection). \DIFdelbegin \DIFdel{However, these works rely on independent simulation loci or genomic regions which prevents the modeling of genome-wide effects of selection as the reduction of effective population size due to the variance of reproductive success of individuals \mbox{%DIFAUXCMD
\citep{Santiago:1995wx} }\hspace{0pt}%DIFAUXCMD
or the combined effects of mutations on individual fitness.
Nevertheless, this }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend strategy brought new insights into the dynamics of selection. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Laval:2019jo} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \Citet{Laval:2019jo} \DIFaddend estimated the number of past selective sweeps in the human genome in the past 100,000 years, their intensity, and their age. \DIFdelbegin \DIFdel{Besides some limitations, these }\DIFdelend \DIFaddbegin \DIFadd{These }\DIFaddend works exemplify the power of likelihood-free methods to infer the complex interaction between demography and selection. \DIFaddbegin \DIFadd{However, all the methods discussed above rely on independent simulation or modeling of loci or genomic regions which prevents the modeling of genome-wide effects of selection as the reduction of effective population size due to the variance of reproductive success of individuals \mbox{%DIFAUXCMD
\citep{Santiago:1995wx} }\hspace{0pt}%DIFAUXCMD
or the combined effects of mutations on individual fitness.
}\DIFaddend 
...
Our model is relatively simple, as it only considered the impact of \DIFaddbegin \DIFadd{new }\DIFaddend beneficial mutations, neglecting the effect of background selection and standing variation. Background selection can mimic directional selection because \DIFdelbegin \DIFdel{they cause }\DIFdelend \DIFaddbegin \DIFadd{it causes }\DIFaddend a similar pattern of diversity reduction around the target of selection \citep{Stephan:2010}\DIFaddbegin \DIFadd{. This }\DIFaddend has been discussed for a long time\DIFdelbegin \DIFdel{. Much }\DIFdelend \DIFaddbegin \DIFadd{; however, much }\DIFaddend less has been discussed about the patterns of background selection on temporal data and their differences with selective sweeps. \citet{Cvijovic:2018} showed that neutral alleles linked to less deleterious backgrounds could quickly rise to high frequencies due to purifying selection, which could mimic the  \DIFdelbegin \DIFdel{temporal }\DIFdelend signal of a selective sweep \DIFaddbegin \DIFadd{in temporal data}\DIFaddend . However, \DIFdelbegin \DIFdel{if background selection is not localized to specific regions of the genome, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Schrider:2020hka} }\hspace{0pt}%DIFAUXCMD
suggests that }\DIFaddend the signal of selective sweeps will be distinct from the effects of background selection \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Schrider:2020hka}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{if background selection is not localized to specific regions of the genome}\DIFaddend . In an attempt to jointly accommodate the effect of demography and selection on the inference of $N_{\mathrm{e}}$, \citet{Johri:2020ee} modeled the effect of background selection and developed an ABC-based approach that jointly estimated the distribution of fitness effects and $N_{\mathrm{e}}$. In their simulations, they modeled deleterious mutations and the classical hard sweep with the inclusion of beneficial mutations. They showed an unbiased estimate of $N_{\mathrm{e}}$ regardless of positive and negative selection presence. Future developments \DIFaddbegin \DIFadd{of our approach }\DIFaddend should include a more realistic genomic architecture were negative and positive mutations can co-occur and explore different concentrations of deleterious mutations. \DIFaddbegin \DIFadd{This will elucidate the importance of background selection in this context, which probably affects our inferences in a way that is difficult to predict with our current results. }\DIFaddend In addition, further developments should explore scenarios of \textit{de novo} mutations and selection acting on standing variation\DIFdelbegin \DIFdel{. It can be achieved with our pipeline and allows }\DIFdelend \DIFaddbegin \DIFadd{, which will allow }\DIFaddend for a more general treatment \DIFdelbegin \DIFdel{of the selection of }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend soft sweeps. The model can also be expanded to more complex demographic scenarios, including changes in population size and genetic exchange with external sources (migration). Including such admixtures will be key in the future development of this approach since it is also a source of heterogeneity in the genome and, thus, might influence the method's performance. 
